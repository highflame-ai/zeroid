...
next-version: ## Print svu-computed next semver from commits since last v* tag
	...
	@echo "next    : $$(svu next)"
	@echo
	@echo "Cut a release: draft a new release in the GitHub UI with tag = svu's recommendation"
	@echo "(or higher). release.yml handles go.mod cross-module ref bumping automatically."
	@echo "See RELEASING.md for the full story."
