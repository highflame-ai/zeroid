...
func (s *IdentityService) ListIdentities(ctx context.Context, accountID, projectID string, identityTypes []string, label, trustLevel, isActive, search string) ([]*domain.Identity, error) {
	return s.repo.List(ctx, accountID, projectID, identityTypes, label, trustLevel, isActive, search)
}
