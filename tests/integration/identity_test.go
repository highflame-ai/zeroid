...
func TestRegisterIdentity(t *testing.T) {
	...
	resp := post(t, "/identities", map[string]any{
...
// TestGetIdentity verifies that GET /identities/{id} returns the identity.
func TestGetIdentity(t *testing.T) {
	externalID := uid("get-agent")
	identity := registerIdentity(t, externalID, []string{"billing:read"})

	resp := get(t, "/identities/"+identity.ID, adminHeaders())
...
func TestGetIdentityNotFound(t *testing.T) {
	resp := get(t, "/identities/00000000-0000-0000-0000-000000000000", adminHeaders())
...
func TestListIdentities(t *testing.T) {
	...
	resp := get(t, "/identities", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body := decode(t, resp)
...
func TestListAgentsFilterByIdentityType(t *testing.T) {
	...
	post(t, "/agents/register", map[string]any{
	...
	resp := get(t, "/agents/registry?identity_type=agent&label=product:guardrails", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body := decode(t, resp)
	...
	resp = get(t, "/agents/registry?identity_type=agent,application&label=product:guardrails", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body = decode(t, resp)
	...
	resp = get(t, "/agents/registry?label=product:guardrails", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body = decode(t, resp)
	all := body["agents"].([]any)
...
// TestUpdateIdentityTrustLevel verifies that PATCH /identities/{id}
// can promote the trust level.
func TestUpdateIdentityTrustLevel(t *testing.T) {
	externalID := uid("trust-agent")
	identity := registerIdentity(t, externalID, []string{"billing:read"})

	resp, err := doRaw(t, http.MethodPatch, "/identities/"+identity.ID, map[string]any{
...
// TestDeleteIdentity verifies that DELETE /identities/{id} deactivates the identity.
func TestDeleteIdentity(t *testing.T) {
	externalID := uid("delete-agent")
	identity := registerIdentity(t, externalID, []string{"billing:read"})

	resp, err := doRaw(t, http.MethodDelete, "/identities/"+identity.ID, nil, adminHeaders())
	require.NoError(t, err)
...
func TestListAgentsFilterByTrustLevel(t *testing.T) {
	...
	resp := get(t, "/agents/registry?trust_level=first_party&label=test:trust-filter", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body := decode(t, resp)
	agents := body["agents"].([]any)
	for _, a := range agents {
	...
	resp = get(t, "/agents/registry?trust_level=unverified&label=test:trust-filter", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body = decode(t, resp)
	agents = body["agents"].([]any)
	for _, a := range agents {
...
func TestListAgentsFilterByIsActive(t *testing.T) {
	...
	// Deactivate.
	deactivateResp, err := doRaw(t, http.MethodPost, "/agents/registry/"+agentID+"/deactivate", nil, adminHeaders())
	require.NoError(t, err)
	...
	resp = get(t, "/agents/registry?is_active=true&label=test:active-filter", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body := decode(t, resp)
	agents := body["agents"].([]any)
	for _, a := range agents {
	...
	resp = get(t, "/agents/registry?is_active=false&label=test:active-filter", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body = decode(t, resp)
	agents = body["agents"].([]any)
	for _, a := range agents {
...
func TestListAgentsSearch(t *testing.T) {
	...
	resp := get(t, "/agents/registry?search=Alpha+Research", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body := decode(t, resp)
	agents := body["agents"].([]any)
	found := false
	...
	resp = get(t, "/agents/registry?search="+ext2[:20], adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body = decode(t, resp)
	agents = body["agents"].([]any)
	found = false
...
func TestListAgentsPagination(t *testing.T) {
	...
	resp := get(t, "/agents/registry?limit=2&offset=0&label=pagination:"+paginationLabel, adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body := decode(t, resp)
	...
	resp = get(t, "/agents/registry?limit=2&offset=2&label=pagination:"+paginationLabel, adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body = decode(t, resp)
	...
	resp = get(t, "/agents/registry?limit=2&offset=4&label=pagination:"+paginationLabel, adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body = decode(t, resp)
...
// TestListIdentitiesEndpointFilters verifies that /identities also supports filters.
func TestListIdentitiesEndpointFilters(t *testing.T) {
	ext := uid("id-filter")
	post(t, "/identities", map[string]any{
		"external_id":    ext,
		"trust_level":    "first_party",
		"identity_type":  "application",
		"sub_type":       "chatbot",
	...
	resp := get(t, "/identities?identity_type=application", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body := decode(t, resp)
	...
	// Search by name.
	resp = get(t, "/identities?search=Filterable+App", adminHeaders())
	require.Equal(t, http.StatusOK, resp.StatusCode)
	body = decode(t, resp)
	identities = body["identities"].([]any)
	found := false
