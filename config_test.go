...
// TestValidate_UnsafeDevPrincipalRefusedInProduction — the dev principal makes
// /oauth2/authorize a rubber stamp (any client, no authentication, no consent),
// so production must refuse to start with it set. Same posture as
// attestation.allow_unsafe_dev_stub, which this mirrors.
func TestValidate_UnsafeDevPrincipalRefusedInProduction(t *testing.T) {
	for _, env := range []string{"production", "prod"} {
		t.Run(env, func(t *testing.T) {
			c := baseValidConfig(t)
			c.Server.Env = env
			c.Attestation.AllowUnsafeDevStub = false
			c.Authorize.UnsafeDevPrincipal = UnsafeDevPrincipalConfig{
				AccountID: "acct-x", ProjectID: "proj-x", UserID: "dev@example.test",
			}

			err := c.Validate()
			if err == nil {
				t.Fatal("production must refuse to start with authorize.unsafe_dev_principal set")
			}
			if !strings.Contains(err.Error(), "unsafe_dev_principal") {
...
// TestValidate_UnsafeDevPrincipalAllowedInDev keeps the escape hatch usable
// where it is meant to be used.
func TestValidate_UnsafeDevPrincipalAllowedInDev(t *testing.T) {
	c := baseValidConfig(t)
	...
	if err := c.Validate(); err != nil {
		t.Fatalf("development should accept the dev principal: %v", err)
	}
}

// TestUnsafeDevPrincipal_PartialConfigIsDisabled — all three tenant fields are
// required. A half-filled block must not authorize into a defaulted tenant.
func TestUnsafeDevPrincipal_PartialConfigIsDisabled(t *testing.T) {
	cases := map[string]UnsafeDevPrincipalConfig{
		"empty":         {},
		"no account_id": {ProjectID: "p", UserID: "u"},
		"no project_id": {AccountID: "a", UserID: "u"},
		"no user_id":    {AccountID: "a", ProjectID: "p"},
	}
	for name, cfg := range cases {
		t.Run(name, func(t *testing.T) {
			if cfg.Enabled() {
				t.Fatalf("%s must not enable the dev principal", name)
			}
		})
	}
	full := UnsafeDevPrincipalConfig{AccountID: "a", ProjectID: "p", UserID: "u"}
	if !full.Enabled() {
		t.Fatal("a fully-specified principal must be enabled")
	}
}
